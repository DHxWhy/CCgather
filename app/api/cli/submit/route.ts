...
export async function POST(request: NextRequest) {
  try {
    ...
    // Harden untrusted input at the boundary. `body` is request.json() cast to
    // SubmitPayload with NO runtime validation, so a non-object dailyUsage entry
    // (e.g. [null]) would throw on property access in every consumer below —
    // starting with the date-format check (day.date), then getPrimaryModel
    // (day.models), the per-day record builder, the STEP 1.5/1.7 date maps, and
    // the badge history map. Drop such entries here, before the first consumer,
    // so no step can 500. The official CLI never emits these; surviving entries
    // are still validated for content downstream.
