...
function displayResults(data: CCGatherData): void {
  ...
  console.log();
  console.log(gray('  ┌─────────────────────────────────────────────────────┐'));
  console.log(gray('  │') + white('  📊 Usage Summary') + gray('                                   │'));
  console.log(gray('  ├─────────────────────────────────────────────────────┤'));
  console.log(gray('  │') + `  Total Tokens:   ${orange(formatNumber(data.usage.totalTokens).padEnd(15))}` + gray('            │'));
  console.log(gray('  │') + `  Total Cost:     ${green('$' + data.usage.totalCost.toFixed(2).padEnd(14))}` + gray('            │'));
  console.log(gray('  │') + `  Input Tokens:   ${white(formatNumber(data.usage.inputTokens).padEnd(15))}` + gray('            │'));
  console.log(gray('  │') + `  Output Tokens:  ${white(formatNumber(data.usage.outputTokens).padEnd(15))}` + gray('            │'));
  console.log(gray('  ├─────────────────────────────────────────────────────┤'));
  console.log(gray('  │') + white('  📈 Stats') + gray('                                           │'));
