...
export const TopThreeCard = forwardRef<HTMLDivElement, TopThreeCardProps>(
  (
    { className, rank, username, avatarUrl, country, countryFlag, tokens, cost, level, ...props },
    ref
  ) => {
      ...
      >
        ...
        >
          ...
          <div className="flex justify-center mb-4">
            <div className={cn("relative", isFirst ? "w-20 h-20" : "w-16 h-16")}>
              {avatarUrl ? (
                <Image
                  src={avatarUrl}
                  alt={username}
                  ...
                  className={cn("rounded-full object-cover", "border-2", styles.border)}
                />
              ) : (
                <div
                  className={cn(
          ...
          {level && (
            <div className="flex justify-center mt-2">
              <span className="text-sm text-[var(--color-text-secondary)]">Lv.{level}</span>
            </div>
          )}
          ...
          <div className="mt-4 pt-4 border-t border-[var(--border-default)]">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="font-mono font-bold text-[var(--color-text-primary)]">{tokens}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-[var(--color-text-muted)]">Cost</span>
              <span className="font-mono text-[var(--color-text-secondary)]">{cost}</span>
            </div>
          </div>
        </div>
      </div>
