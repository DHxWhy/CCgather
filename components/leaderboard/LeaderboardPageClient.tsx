...
const ITEMS_PER_PAGE = LEADERBOARD_PAGE_SIZE;
...
export function LeaderboardPageClient({ initialLeaderboard }: LeaderboardPageClientProps) {
  // 서버가 HTML 에 담아 보낸 첫 페이지 — 기본 필터(30일·전체)일 때만 초기 상태로 쓴다
  const ssr = initialLeaderboard?.period === LEADERBOARD_DEFAULT_PERIOD ? initialLeaderboard : null;
  ...
  const [pagesData, setPagesData] = useState<Map<number, DisplayUser[]>>(() =>
    ssr ? new Map([[1, toDisplayUsers(ssr.users, 1, ssr.period, null)]]) : new Map()
  );
  ...
  const [total, setTotal] = useState(ssr?.pagination.total ?? 0);
  const [totalPages, setTotalPages] = useState(ssr?.pagination.totalPages ?? 1);
  ...
  const fetchPage = useCallback(
    async (
      ...
      const data = await response.json();
      const transformedUsers = toDisplayUsers(
        data.users || [],
        pageNum,
        periodFilter,
