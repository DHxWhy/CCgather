...
export function ActivityHeatmap({ data, periodDays = 90 }: ActivityHeatmapProps) {
  ...
  return (
    <div className="space-y-2 overflow-visible">
      {/* Month labels */}
      <div className="flex text-[9px] text-[var(--color-text-muted)] pl-8 relative h-4 overflow-visible">
            ...
            return (
              <div key={week} className="flex flex-col gap-[3px]">
                {Array.from({ length: 7 }, (_, dayIndex) => {
                  ...
                  const verticalClass = tooltipPos.vertical === 'top' ? 'bottom-full mb-2' : 'top-full mt-2';
                  ...
                  const arrowVerticalClass = tooltipPos.vertical === 'top' ? 'top-full -mt-px' : 'bottom-full -mb-px';
                      ...
                      {isHovered && (
                        ...
                        >
                          ...
                          <div className={`absolute ${arrowVerticalClass} ${arrowHorizontalClass}`}>
                            <div className={`border-4 border-transparent ${arrowBorderClass}`} />
                          </div>
                        </div>
                      )}
                    </div>
      ...
      <div className="flex items-center justify-end gap-1.5 text-[9px] text-[var(--color-text-muted)]">
        ...
        {INTENSITY_COLORS.map((color, i) => (
          <div
            key={i}
            className={`w-[11px] h-[11px] rounded-sm ${color}`}
          />
        ))}
        <span>More</span>
      </div>
